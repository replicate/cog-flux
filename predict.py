...
class SharedInputs:
    ...
    profile_python: Input = Input(
        description="run cprofile over the whole entire codebase",
        default=False,
    )
    profile_torch: Input = Input(
        description="run torch profiler over the whole entire codebase",
        default=False,
    )
...
class SchnellPredictor(Predictor):
    ...
    def predict(
        ...
        profile_torch: bool = SHARED_INPUTS.profile_torch,
        profile_python: bool = SHARED_INPUTS.profile_python,
    ) -> List[Path]:
