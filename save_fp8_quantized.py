...
def generate_base_img(p, img_name='cool_dog_1234.png'):
    out = p.predict('a cool dog', '1:1', None, 0, 1, 28, 3, 1234, 'png', 100, True, True, "1")
    # for f in out:
    #     f.save('cool_dog_1234.png')
...
def save_dev_fp8():
    ...
    #     seed=10,
    # )
    # if p.fp8_pipe.config.version == ModelVersion.flux_schnell:
    #     print("schnell")
    #     warmup_dict["num_steps"] = 4
    #     for _ in range(3):
    #         p.fp8_pipe.generate(**warmup_dict)
    # else:
    #     print("dev")
    #     p.fp8_pipe.generate(**warmup_dict)
...
# ok. so if that's saved, then we need to load it. 
# what does that look like? changing the config, I believe. 
def test_dev_fp8():
    p = DevPredictor()
    ...
    # p.fp8_pipe.generate(**warmup_dict)
    #p.compile_fp8()
    generate_base_img(p, 'cool_dog_1234_loaded_from_compiled.png')

def test_dev_base(i):
    p = DevPredictor()
    p.setup()
    generate_base_img(p, f'cool_dog_1234_compiled_{i}.png')
